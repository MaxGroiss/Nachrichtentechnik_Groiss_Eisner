...
plt.plot(x, calculate_ebdn0(True, B_1, p_r, n_0, db_list)[1], 'r--', label=r'$W_eff$ in bits/s/Hz (S-band)')
plt.plot(x, calculate_ebdn0(True, B_2, p_r, n_0, db_list)[0], 'g-', label=r'$E_b/N_0$ in dB (VHF)')
plt.plot(x, calculate_ebdn0(True, B_2, p_r, n_0, db_list)[1], 'g--', label=r'$W_eff$ in bits/s/Hz (VHF)')
plt.axhline(y=1, color='y', linestyle='-', label="1 bits/s/Hz")
plt.title(r'Transferverhalten über die Wochentage')
plt.ylabel(r'$E_b/N_0$ in dB und $W_eff$ in bits/s/Hz')
plt.xlabel(r'Weekdays')
